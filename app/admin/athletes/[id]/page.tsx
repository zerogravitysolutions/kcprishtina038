...
type EntryRow = EntryLike & {
  ...
  rpe: number | null;
  notes: string | null;
  ride: { id: string; ride_date: string; title: string | null; focus: string | null; kind: "group" | "solo" } | null;
};
...
export default async function AthleteProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: athleteData }, { data: profileData }, { data: entryData }] = await Promise.all([
    ...
    supabase
      .from("ride_entries")
      .select("id, participated, distance_km, moving_seconds, elevation_m, avg_hr, max_hr, avg_power_w, np_w, ftp_w, best_power_1m_w, best_power_3m_w, best_power_5m_w, best_power_10m_w, best_power_20m_w, best_power_60m_w, tss, rpe, notes, ride:training_rides(id, ride_date, title, focus, kind)")
      .eq("athlete_id", id),
  ]);
            ...
            {recent.length === 0 ? (
              ...
              recent.map((e) => (
                <tr key={e.id}>
                  ...
                  <td>{e.ride ? <Link href={`/admin/training/${e.ride.id}`} style={{ fontWeight: 600 }}>{e.ride.title || e.ride.focus || (e.ride.kind === "solo" ? "Individuale" : "Grup")}</Link> : "—"}</td>
