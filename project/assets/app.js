...
(function tweaks() {
  ...
  function makePanel() {
    ...
    wrap.style.cssText = `
      ...
      background: #0F1A2E; color: #F4F2EC; border-radius: 14px;
      padding: 18px; width: 280px;
      font-family: "Manrope", sans-serif;
      box-shadow: 0 20px 40px rgba(15,26,46,.25);
      display: none;
    `;
    wrap.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        <div style="font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:#A4ADB6;">Tweaks</div>
        <button id="kc-tw-x" style="background:transparent;color:#F4F2EC;border:0;cursor:pointer;font-size:18px;line-height:1;padding:0;">×</button>
      </div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:.16em;text-transform:uppercase;color:#A4ADB6;margin-bottom:8px;">Language</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
        <button data-tw-lang="sq" class="kc-tw-btn">Shqip</button>
        <button data-tw-lang="en" class="kc-tw-btn">English</button>
      </div>
    `;
    ...
    style.textContent = `
      .kc-tw-btn {
        background: #1B2742; color: #F4F2EC;
        border: 1px solid rgba(244,242,236,.14);
        padding: 10px 12px; border-radius: 8px; cursor: pointer;
        font-family: "Manrope", sans-serif; font-size: 13px; font-weight: 600;
      }
      .kc-tw-btn.active { background: #C25A2D; border-color: #C25A2D; }
    `;
    document.head.appendChild(style);

    wrap.querySelectorAll("[data-tw-lang]").forEach(b => {
      b.addEventListener("click", () => {
        LangState.set(b.dataset.twLang);
        syncBtns();
        try {
          window.parent.postMessage({ type: "__edit_mode_set_keys", edits: { lang: b.dataset.twLang } }, "*");
        } catch (e) {}
      });
    });
    wrap.querySelector("#kc-tw-x").addEventListener("click", () => {
      active = false;
    ...
    function syncBtns() {
      wrap.querySelectorAll("[data-tw-lang]").forEach(b => {
        b.classList.toggle("active", b.dataset.twLang === LangState.get());
      });
    }
    LangState.onChange(syncBtns);
    syncBtns();
    return wrap;
  }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (d.type === "__activate_edit_mode") {
      ...
      active = true; panel.style.display = "block";
    } else if (d.type === "__deactivate_edit_mode") {
      active = false; if (panel) panel.style.display = "none";
    }
  });
  try { window.parent.postMessage({ type: "__edit_mode_available" }, "*"); } catch (e) {}

  // Honor persisted default
  if (TWEAK_DEFAULTS.lang && !localStorage.getItem("kc038_lang")) {
    LangState.set(TWEAK_DEFAULTS.lang);
  }
})();
